...
class FailOpenMiddleware(BaseMiddleware):
    ...
    Swallows backend errors so application logic continues without cache.
    Read operations return ``default``; writes become no-ops; counters fall
    back to an in-process best-effort counter.
    """

    def __init__(self, cache: SyncCache, *, log_errors: bool = True, log_tracebacks: bool = False) -> None:
        super().__init__(cache)
        self._cache: SyncCache = cache
        ...
        *,
        ...
        try:
            if serializer is MISSING:
                ...
                self._cache.set(key, value, ttl=ttl, serializer=serializer)  # type: ignore[call-arg]
        except Exception as e:
    ...
    def incr(self, key: str, *, delta: int = 1, ttl_if_new: int | timedelta | None = None) -> int:
        try:
            return int(self._cache.incr(key, delta=delta, ttl_if_new=ttl_if_new))
        except Exception as e:
            self._log_error(
                "Cache incr failed for key '%s', using local counter fallback: %s: %s",
                key,
                type(e).__name__,
                str(e),
            )
            self._local_counters[key] = int(self._local_counters.get(key, 0)) + int(delta)
    ...
    def decr(self, key: str, *, delta: int = 1) -> int:
        try:
            ...
            self._log_error(
                "Cache decr failed for key '%s', using local counter fallback: %s: %s",
                key,
                type(e).__name__,
                str(e),
            )
    ...
    def health(self) -> HealthStatus:
        try:
            ...
            self._log_error("Cache health check failed: %s: %s", type(e).__name__, str(e))
            return {"healthy": False, "latency_ms": 0.0, "backend": "fail-open"}

    def healthy(self) -> bool:
        try:
            return bool(self._cache.healthy())
        except Exception as e:
            self._log_error("Cache healthy check failed: %s: %s", type(e).__name__, str(e))
            return False
...
class AsyncFailOpenMiddleware(BaseMiddleware):
        ...
        *,
        ...
        try:
            if serializer is MISSING:
                ...
                await self._cache.set(key, value, ttl=ttl, serializer=serializer)  # type: ignore[call-arg]
        except Exception as e:
    ...
    async def incr(self, key: str, *, delta: int = 1, ttl_if_new: int | timedelta | None = None) -> int:
        try:
            return int(await self._cache.incr(key, delta=delta, ttl_if_new=ttl_if_new))
        except Exception as e:
            self._log_error(
                "Async cache incr failed for key '%s', using local counter fallback: %s: %s",
                key,
                type(e).__name__,
                str(e),
            )
    ...
    async def decr(self, key: str, *, delta: int = 1) -> int:
        try:
            ...
            self._log_error(
                "Async cache decr failed for key '%s', using local counter fallback: %s: %s",
                key,
                type(e).__name__,
                str(e),
            )
    ...
    async def clear(self, *, all: bool = False) -> None:
        try:
            await self._cache.clear(all=all)
        except Exception as e:
    ...
    async def health(self) -> HealthStatus:
        try:
            ...
            self._log_error("Async cache health check failed: %s: %s", type(e).__name__, str(e))
            return {"healthy": False, "latency_ms": 0.0, "backend": "fail-open"}

    async def healthy(self) -> bool:
        try:
            return bool(await self._cache.healthy())
        except Exception as e:
            self._log_error("Async cache healthy check failed: %s: %s", type(e).__name__, str(e))
            return False
