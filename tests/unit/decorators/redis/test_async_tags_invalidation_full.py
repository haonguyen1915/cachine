...
@pytest.mark.asyncio
async def test_async_tags_full_invalidation(redis_async_cache):
    ...
    removed = await cache.invalidate_tags(["role:admin"])  # type: ignore[attr-defined]
    assert removed >= 1
    assert await get_user(1) == {"id": 1, "role": "admin"}
    assert await get_user(2) == {"id": 2, "role": "member"}
    assert calls["n"] == 3
    ...
    removed = await cache.invalidate_tags(["users", "user:2"])  # type: ignore[attr-defined]
    assert removed >= 1
    assert await get_user(1) == {"id": 1, "role": "admin"}
    assert await get_user(2) == {"id": 2, "role": "member"}
    assert calls["n"] == 5
