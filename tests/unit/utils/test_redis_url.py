...
class TestParseClusterRedisURL:
    ...
    def test_cluster_url_basic(self) -> None:
        ...
        assert config["type"] == "cluster"
        assert len(config["nodes"]) == 3
        assert config["nodes"][0] == {"host": "node1", "port": 7000}
        assert config["nodes"][1] == {"host": "node2", "port": 7001}
        assert config["nodes"][2] == {"host": "node3", "port": 7002}
        assert config["password"] is None
        assert config["ssl"] is False
