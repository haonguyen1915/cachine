...
@pytest.mark.asyncio
async def test_async_decorator_condition_and_cache_none(a_redis_cache: AsyncRedisCache) -> None:
    cache = a_redis_cache
    calls = {"n": 0}
...
@pytest.mark.asyncio
async def test_async_decorator_tags_and_invalidation(a_redis_cache: AsyncRedisCache) -> None:
    cache = a_redis_cache
